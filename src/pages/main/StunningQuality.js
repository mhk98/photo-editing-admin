...
const StunningQuality = () => {
  ...
  const [updateStunningQuality] = useUpdateStunningQualityMutation();
  ...
  const { data, isLoading, isError, error } = useGetAllStunningQualityQuery();
  ...
  const handleDeleteHomeBanner = async (id) => {
    const res = await deleteStunningQuality(id);

...
} from "../../features/howItWorks/howItWorks";
...
const HowItWorks = () => {
  ...
  const [createHowItWorks] = useCreateHowItWorksMutation();
  ...
  const onFormSubmit = async () => {
    ...
    try {
      const res = await createHowItWorks(formData);
      if (res) {
  ...
  const onFormEdit = async () => {
    ...
    try {
      const res = await updateHowItWorks({ id: updateId, data: formData });
      if (res) {
  ...
  const { data, isLoading, isError, error } = useGetAllHowItWorksQuery();
  ...
  const handleDeleteHomeBanner = async (id) => {
    const res = await deleteHowItWorks(id);
          ...
          <div className="mc-card">
            ...
            <Modal show={show} onHide={handleClose}>
              <Modal.Header closeButton className="text-center p-3">
                <Modal.Title className="w-100">Home How It Works</Modal.Title>
              </Modal.Header>

...
} from "../../features/pricingPlan/pricingPlan";

const PricingPlan = () => {
  const thead = ["image1", "image2", "image3", "image4", "action"];
  ...
  const onFormEdit = async () => {
    ...
    try {
      const res = await updatePricingPlan({ id: updateId, data: formData });
      if (res) {
  ...
  const { data, isLoading, isError, error } = useGetAllPricingPlanQuery();
          ...
          <div className="mc-card">
            ...
            <Modal show={show} onHide={handleClose}>
              <Modal.Header closeButton className="text-center p-3">
                <Modal.Title className="w-100">Home Pricing Plan</Modal.Title>
              </Modal.Header>
